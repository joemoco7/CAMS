...
class DatabaseManager:
    ...
    def close(self):
        """A new method to cleanly close the connection."""
        if self.conn:
            self.conn.close()
            logging.info("SyncManager's database connection closed.")
...
class SyncManager:
    ...
    SYNC_POLL_INTERVAL_SECONDS = 60 # Sync less frequently
    ...
    def __init__(self, app_instance):
        ...
        self.last_sync_utc = self._load_last_sync_time()
    ...
    def _get_or_create_program_id(self) -> Tuple[str, Path]:
        ...
        if 'program_id' not in settings:
            settings['program_id'] = str(uuid.uuid4())
        # We can also store the server address here for easy configuration
        if 'server_address' not in settings:
            settings['server_address'] = "" # e.g., "http://192.168.1.100:5678"
        with open(settings_path, 'w', encoding='utf-8') as f:
            json.dump(settings, f, indent=4)
        
        if settings['server_address']:
            self.server_url = settings['server_address']
            logging.info(f"Loaded server URL from settings: {self.server_url}")
    ...
    def _load_last_sync_time(self):
        try:
            with open(self.sync_settings_path, 'r') as f:
                return json.load(f).get('last_sync_utc', '1970-01-01T00:00:00.000000+00:00')
        except (FileNotFoundError, json.JSONDecodeError):
            return '1970-01-01T00:00:00.000000+00:00'

    def _save_last_sync_time(self, timestamp: str):
        # (Implementation similar to _get_or_create_program_id)
        ...

    def queue_upload(self, member_records: List = None, attendance_records: List = None):
        """Public method to add pending changes to the upload queue."""
        payload = {
            "program_id": self.program_id,
            "members": member_records or [],
    ...
    def _uploader_thread_target(self):
        while True:
            ...
            if not self.server_url:
                logging.warning("No server connection. Upload will be attempted on next sync cycle.")
                time.sleep(10) # Wait before retrying
                self.upload_queue.put(payload) # Put it back in the queue
                continue
            
            try:
                url = f"{self.server_url}/batch_update"
                response = requests.post(url, json=payload, timeout=15)
                response.raise_for_status()
                logging.info(f"Successfully pushed batch update to the server.")
            except requests.exceptions.RequestException as e:
                logging.error(f"Failed to push batch update to server: {e}. Re-queuing.")
                time.sleep(10)
                self.upload_queue.put(payload)
    ...
    def discover_server(self):
        logging.info("Attempting to discover server on the network...")
        self.signals.status_updated.emit("Discovering Server...", "#0076c6")
        zeroconf = Zeroconf()
        class ServiceListener:
            def __init__(self, sync_manager): self.sync_manager = sync_manager
            def add_service(self, zc, type, name):
                info = zc.get_service_info(type, name)
                if info and not self.sync_manager.server_url:
                    address = socket.inet_ntoa(info.addresses[0])
                    port = info.port
                    self.sync_manager.server_url = f"http://{address}:{port}"
                    logging.info(f"Discovered and set server URL: {self.sync_manager.server_url}")
        
        ServiceBrowser(zeroconf, self.SERVICE_TYPE, ServiceListener(self))
        time.sleep(5) # Discover for 5 seconds
        zeroconf.close()
        if not self.server_url:
            self.signals.status_updated.emit("Offline (No Server Found)", "#c62828")

    def _sync_loop_thread_target(self):
        """The main background loop that periodically syncs."""
        while self.sync_active:
            if self.server_url:
                self.signals.status_updated.emit("Syncing...", "#0076c6")
                try:
                    self._push_all_pending_changes()
                    self._pull_and_reconcile_changes()
                    self.signals.status_updated.emit(f"Synced @ {datetime.now():%H:%M}", "#2e7d32")
                except requests.exceptions.RequestException as e:
                    logging.error(f"Sync failed: {e}")
                    self.signals.status_updated.emit("Connection Lost", "#c62828")
            else:
                self.discover_server()

            time.sleep(self.SYNC_POLL_INTERVAL_SECONDS)

    def _push_all_pending_changes(self):
        """Finds all local changes since last sync and queues them for upload."""
        # This can be made more robust, but for now the uploader thread handles it
        pass

    def _pull_and_reconcile_changes(self):
        """Gets updates from the server and intelligently merges them."""
        response = requests.get(f"{self.server_url}/get_updates_since", params={'timestamp': self.last_sync_utc}, timeout=20)
        response.raise_for_status()
        ...
        # --- MODIFIED: Uses its own self.cursor and self.conn ---
        changes_found = False
        # Members
        for mid, s_member in server_members.items():
            l_member = self.app.members.get(mid)
            if not l_member or s_member.get('last_modified_utc', '') > l_member.get('last_modified_utc', ''):
                changes_found = True
                self.cursor.execute("""
                    REPLACE INTO members (id, name, first_name, last_name, roles, first_visit, last_modified_utc, last_modified_by) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    mid, s_member.get('name'), s_member.get('first_name'), s_member.get('last_name'), 
                    json.dumps(s_member.get('roles', [])), s_member.get('first_visit'), 
                    s_member.get('last_modified_utc'), s_member.get('last_modified_by')
                ))
                self.app.members[mid] = s_member
        
        # Attendance
        for rid, s_record in server_attendance.items():
            if rid not in self.app.attendance:
                changes_found = True
                self.cursor.execute("""
                    REPLACE INTO attendance (record_id, name, date, time, program_id, is_deleted, deleted_by, deleted_timestamp_utc) 
                    VALUES (?, ?, ?, ?, ?, ?, ?, ?)
                """, (
                    rid, s_record.get('name'), s_record.get('date'), s_record.get('time'), s_record.get('program_id'),
                    1 if s_record.get('is_deleted') else 0, s_record.get('deleted_by'), s_record.get('deleted_timestamp_utc')
                ))
                self.app.attendance[rid] = s_record

        self.conn.commit()
        self._save_last_sync_time(server_state.get('server_time'))
        
        if changes_found:
            self.signals.ui_refresh_needed.emit()
...
class MainWindow(QMainWindow):
    ...
    def __init__(self, parent=None, db_manager=None):
        ...
        # --- MODIFIED: Pass the database PATH, not the full object ---
        self.sync_manager = SyncManager(self, db_manager.db_path)
        # ---
    ...
    def start_sync_discovery(self):
        """Starts the self-managing sync manager."""
        logging.info("Handing off control to the background SyncManager.")
    ...
    def _handle_sync_ui_refresh(self):
        logging.info("UI refresh triggered by SyncManager. Reloading data from DB.")
        
        # First, reload the in-memory data from the database that SyncManager updated
        self.load_data() 
    ...
    def init_ui(self):
        ...
        self.setStyleSheet(f"""
            ...
            /* --- MODIFIED & NEW STYLES --- */
            QFrame#TileGridContainer {{
                border: 1px solid {self.colors['border']};
                border-radius: 8px;
            }}
            QPushButton[class="name-tile"] {{
                background-color: {self.colors['white']};
                color: {self.colors['primary']};
                font-size: 14pt; /* Slightly smaller for better fit */
                font-weight: bold;
            ...
            QPushButton[class="name-tile"]:hover {{
                background-color: #f5f5f5;
                border-color: {self.colors['secondary']};
            }}
            QPushButton[class="name-tile"]:pressed {{
                background-color: {self.colors['secondary']};
                color: white;
            }}
            QPushButton[class="alpha-filter"] {{
                font-size: 12pt;
                font-weight: bold;
                min-width: 35px;
                max-width: 35px;
            ...
            QPushButton[class="alpha-filter"]:checked, QPushButton[class="alpha-filter"]:hover {{
                background-color: {self.colors['secondary']};
                color: white;
                border-color: {self.colors['primary']};
            }}
            QPushButton[class="page-nav"] {{
                background-color: {self.colors['white']};
                color: {self.colors['primary']};
                border: 1px solid {self.colors['secondary']};
            }}
            QPushButton[class="page-nav"]:disabled {{
                background-color: #f0f0f0;
                color: #aaa;
                border-color: #ddd;
            }}
    ...
    def _set_alpha_filter(self, letter: str):
        self.current_alpha_filter = letter
        self.update_checkin_tile_view()
    ...
    def _on_search_text_changed(self, text: str):
        # This method provides the intelligent search behavior
        if text:
            # When typing, always search all members, overriding the letter filter
            self.current_alpha_filter = "All"
            first_char = text[0].upper()
            if first_char in self.alpha_buttons:
                # Visually check the button but don't filter by it
                self.alpha_buttons[first_char].setChecked(True)
        else:
            # When cleared, go back to the "All" view
            self.alpha_buttons["All"].setChecked(True)
        
        self.update_checkin_lists()

    def _set_alpha_filter(self, letter: str):
        # This method is for when a user explicitly clicks a letter button
        self.checkin_search_edit.clear() # Clear search when a letter is clicked
        self.current_alpha_filter = letter
        self.current_page_number = 1 # Reset to page 1
        self.update_checkin_tile_view()

    def create_manual_checkin_page(self) -> QWidget:
        # --- Main Container and Tab Widget ---
        page_container = QWidget()
        outer_layout = QVBoxLayout(page_container)
        outer_layout.setContentsMargins(40, 20, 40, 20)
        outer_layout.setSpacing(15)

        title = QLabel("Returnee Check-in")
        title.setStyleSheet(f"font-size:22px; font-weight:bold; color:{self.colors['primary']};")
        outer_layout.addWidget(title, alignment=Qt.AlignCenter)

        self.checkin_search_edit = QLineEdit()
        self.checkin_search_edit.setPlaceholderText("Type a name to search...")
        self.checkin_search_edit.setStyleSheet("font-size: 16px; padding: 12px; border-radius: 6px;")
        self.checkin_search_edit.textChanged.connect(self._on_search_text_changed) # Use new handler
        outer_layout.addWidget(self.checkin_search_edit)
        
        tabs = QTabWidget()
        outer_layout.addWidget(tabs)

        # --- TAB 1: Tile View ---
        tile_view_widget = QWidget()
        tile_view_layout = QVBoxLayout(tile_view_widget)
        
        # Add a container frame for better visual structure
        grid_container = QFrame()
        grid_container.setObjectName("TileGridContainer")
        grid_container_layout = QVBoxLayout(grid_container)
        tile_view_layout.addWidget(grid_container)

        # Alphabet Filter
        alpha_filter_widget = QWidget()
        alpha_filter_layout = QHBoxLayout(alpha_filter_widget)
        alpha_filter_layout.setSpacing(5)
        
        self.alpha_button_group = QButtonGroup(self)
        self.alpha_button_group.setExclusive(True)
        
        self.alpha_buttons = {} # To easily find buttons later
        all_btn = QPushButton("All")
        all_btn.setProperty("class", "alpha-filter")
        all_btn.setCheckable(True)
        all_btn.setChecked(True)
        all_btn.clicked.connect(lambda: self._set_alpha_filter("All"))
        self.alpha_button_group.addButton(all_btn)
        alpha_filter_layout.addWidget(all_btn)
        self.alpha_buttons["All"] = all_btn
        alpha_filter_layout.addStretch()

        for letter in "ABCDEFGHIJKLMNOPQRSTUVWXYZ":
            btn = QPushButton(letter)
            btn.setProperty("class", "alpha-filter")
            btn.setCheckable(True)
            btn.clicked.connect(lambda _, l=letter: self._set_alpha_filter(l))
            self.alpha_button_group.addButton(btn)
            alpha_filter_layout.addWidget(btn)
            self.alpha_buttons[letter] = btn

        self.current_alpha_filter = "All"
        grid_container_layout.addWidget(alpha_filter_widget)
        ...
        self.checkin_tile_layout = QGridLayout(tile_container)
        self.checkin_tile_layout.setSpacing(15)
        self.checkin_tile_layout.setAlignment(Qt.AlignTop)
        scroll_area.setWidget(tile_container)
        grid_container_layout.addWidget(scroll_area)

        # --- Pagination Controls ---
        self.page_nav_widget = QWidget()
        page_nav_layout = QHBoxLayout(self.page_nav_widget)
        self.prev_page_btn = QPushButton("⬅ Previous")
        self.prev_page_btn.setProperty("cssClass", "page-nav")
        self.prev_page_btn.clicked.connect(self._go_to_prev_page)
        self.page_nav_label = QLabel("Page 1 of 1")
        self.page_nav_label.setAlignment(Qt.AlignCenter)
        self.next_page_btn = QPushButton("Next ➡")
        self.next_page_btn.setProperty("cssClass", "page-nav")
        ...
        page_nav_layout.addWidget(self.page_nav_label)
        page_nav_layout.addStretch()
        page_nav_layout.addWidget(self.next_page_btn)
        grid_container_layout.addWidget(self.page_nav_widget)
        self.page_nav_widget.setVisible(False) # Initially hidden

        tabs.addTab(tile_view_widget, "Tile View")

        # --- TAB 2: List View (Original Layout) ---
        list_view_widget = QWidget()
        list_view_layout = QVBoxLayout(list_view_widget)
        list_view_layout.setSpacing(15)
        splitter = QSplitter(Qt.Horizontal)
        left = QWidget(); left_layout = QVBoxLayout(left)
        left_layout.addWidget(QLabel("All Members", alignment=Qt.AlignCenter))
        self.all_members_list = QListWidget(); left_layout.addWidget(self.all_members_list)
        splitter.addWidget(left)
        right = QWidget(); right_layout = QVBoxLayout(right)
        right_layout.addWidget(QLabel("Currently Checked In", alignment=Qt.AlignCenter))
        self.checked_in_list = QListWidget(); right_layout.addWidget(self.checked_in_list)
        splitter.addWidget(right); list_view_layout.addWidget(splitter)
        bottom = QHBoxLayout(); checkin_btn = QPushButton("Check In Selected"); checkin_btn.setProperty("cssClass", "primary"); remove_btn = QPushButton("Remove Check-in"); remove_btn.setProperty("cssClass", "secondary")
        bottom.addStretch(); bottom.addWidget(checkin_btn); bottom.addWidget(remove_btn); bottom.addStretch(); list_view_layout.addLayout(bottom)
        checkin_btn.clicked.connect(self.handle_checkin_selected); remove_btn.clicked.connect(self.handle_remove_checkin)
        tabs.addTab(list_view_widget, "List View")
        
        # --- Back Button ---
        back_btn = QPushButton("Back to Home")
        back_btn.clicked.connect(lambda: self.stack.setCurrentWidget(self.home_page))
        outer_layout.addWidget(back_btn, alignment=Qt.AlignLeft)
        
        # --- Timers and Connections ---
        self.checked_in_list_timer = QTimer(self)
        self.checked_in_list_timer.timeout.connect(self.update_checked_in_list_display)
        self.stack.currentChanged.connect(lambda idx: (self.checked_in_list_timer.start(30000), self.update_checkin_lists()) if self.stack.widget(idx) == page_container else self.checked_in_list_timer.stop())
    ...
    def handle_profile_save(self, member_id, data):
        ...
        data['last_modified_utc'] = datetime.now(timezone.utc).isoformat()
        data['last_modified_by'] = self.sync_manager.program_id
        
        # --- CRITICAL FIX: WRITE TO LOCAL DB IMMEDIATELY ---
        try:
            self.db.cursor.execute("""
                UPDATE members SET name=?, first_name=?, last_name=?, roles=?, last_modified_utc=?, last_modified_by=?
                WHERE id=?
            """, (data['name'], data['first_name'], data['last_name'], json.dumps(data['roles']), 
                data['last_modified_utc'], data['last_modified_by'], member_id))
            self.db.conn.commit()
            
            self.members[member_id].update(data)
            logging.info(f"Locally saved profile update for {data['name']}.")
            self.populate_profile_list()
        except Exception as e:
            logging.error(f"Failed to update member locally: {e}")
            return

        # Queue the upload
        member_data_to_sync = self.members[member_id]
        self.sync_manager.queue_upload(member_records=[member_data_to_sync])
        QMessageBox.information(self, "Saved", "Profile saved locally. Syncing in background.")
    ...
    def record_attendance(self, name: str) -> bool:
        ...
        # Check against local data first
        todays_checkins = [r for r in self.attendance.values() if r.get('date') == today_str and not r.get('is_deleted')]
        ...
        new_record = {
            "record_id": str(uuid.uuid4()),
            "program_id": self.sync_manager.program_id,
            "name": name, "date": today_str, "time": now_time.strftime('%H:%M:%S'),
            "is_deleted": False, "deleted_by": None, "deleted_timestamp_utc": None
        }
        ...
        try:
            self.db.cursor.execute("""
                INSERT INTO attendance (record_id, name, date, time, program_id)
                VALUES (?, ?, ?, ?, ?)
            """, (new_record['record_id'], new_record['name'], new_record['date'], new_record['time'], new_record['program_id']))
            self.db.conn.commit()
            
            # Also update the in-memory dictionary
            self.attendance[new_record['record_id']] = new_record
            logging.info(f"Locally saved check-in for {name}.")
            self.update_checkin_lists() # Refresh UI immediately
        except Exception as e:
            logging.error(f"Failed to save attendance locally: {e}")
            QMessageBox.critical(self, "Database Error", "Could not save the check-in locally. Please restart the application.")
            return False
        
        # Now, ask the sync manager to upload this change in the background
        self.sync_manager.queue_upload(attendance_records=[new_record])
    ...
    def update_checkin_lists(self):
        # This is now the main dispatcher for UI updates on this page
        self.update_checkin_tile_view()
        self.update_all_members_list()  # For the original List View tab
        self.update_checked_in_list_display()

    def update_checkin_tile_view(self):
        from math import ceil

        # Clear existing tiles
        while self.checkin_tile_layout.count():
            child = self.checkin_tile_layout.takeAt(0)
            if child.widget():
                child.widget().deleteLater()

        search_term = self.checkin_search_edit.text().lower()
        
        # --- 1. Filter members ---
        filtered_members = []
        if search_term:
            # If there's a search term, it takes priority over the letter filter
            for member in self.members.values():
                if search_term in member['name'].lower():
                    filtered_members.append(member)
        else:
            # Otherwise, use the selected alpha filter
            for member in self.members.values():
                if self.current_alpha_filter == "All" or member['name'].lower().startswith(self.current_alpha_filter.lower()):
                    filtered_members.append(member)

        sorted_members = sorted(filtered_members, key=lambda m: (m.get('last_name', ''), m.get('first_name', '')))
        
        # --- 2. Pagination Logic ---
        total_names = len(sorted_members)
        total_pages = ceil(total_names / self.names_per_page)
        if total_pages == 0: total_pages = 1

        # Safety check for current page number
        if self.current_page_number > total_pages:
            self.current_page_number = total_pages

        # Slice the list to get names for the current page
        start_index = (self.current_page_number - 1) * self.names_per_page
        end_index = start_index + self.names_per_page
        members_to_display = sorted_members[start_index:end_index]

        # --- 3. Update Pagination UI ---
        if total_pages > 1:
            self.page_nav_widget.setVisible(True)
            self.page_nav_label.setText(f"Page {self.current_page_number} of {total_pages}")
            self.prev_page_btn.setEnabled(self.current_page_number > 1)
            self.next_page_btn.setEnabled(self.current_page_number < total_pages)
        else:
            self.page_nav_widget.setVisible(False)

        # --- 4. Create and display new tiles ---
        COLUMNS = 5 
        for i, member in enumerate(members_to_display):
            btn = QPushButton(member['name'])
            btn.setProperty("class", "name-tile")
            btn.setToolTip(f"Check in {member['name']}")
            btn.clicked.connect(lambda _, m=member: self.handle_quick_select(m))
            
            row, col = divmod(i, COLUMNS)
            self.checkin_tile_layout.addWidget(btn, row, col)
    ...
    def closeEvent(self, event: QCloseEvent):
        ...
        if hasattr(self, 'sync_manager'):
            # --- ADDED THIS LINE to close the SyncManager's DB connection ---
